...
def evaluate():
    ...
    detpath = os.path.join('predictTxt/baseline/', '{:s}.txt')
...
def yololoss(predicts, labels, lambda_coord=5, lambda_noobj=0.5):
    ...
    loss += (((predicts*obj_mask)[:, 10:, :, :] - (labels*obj_mask)[:, 10:, :, :])**2).sum()
    ...
    loss += lambda_coord * (((predicts*obj_mask*bbx0_mask)[:, 0:2, :, :] - (labels*obj_mask*bbx0_mask)[:, 0:2, :, :]) ** 2).sum()
    loss += lambda_coord * ((((predicts*obj_mask*bbx0_mask)[:, 2:4, :, :]+0.000001)**0.5 - ((labels*obj_mask*bbx0_mask)[:, 2:4, :, :]+0.000001)**0.5) ** 2).sum()
    
    loss += lambda_coord * (((predicts*obj_mask*(1-bbx0_mask))[:, 5:7, :, :] - (labels*obj_mask*(1-bbx0_mask))[:, 0:2, :, :]) ** 2).sum()
    loss += lambda_coord * ((((predicts*obj_mask*(1-bbx0_mask))[:, 7:9, :, :]+0.000001)**0.5 - ((labels*obj_mask*(1-bbx0_mask))[:, 2:4, :, :]+0.000001)**0.5) ** 2).sum()
    
    
    loss += (((predicts*obj_mask*bbx0_mask)[:, 4:5, :, :] - max_iou*obj_mask[:, 0:1, :, :]*bbx0_mask[:, 0:1, :, :]) ** 2).sum()
    loss += lambda_noobj * (((predicts*obj_mask*bbx0_mask)[:, 9, :, :]) ** 2).sum()

    loss += (((predicts*obj_mask*(1-bbx0_mask))[:, 9:10, :, :] - max_iou*obj_mask[:, 0:1, :, :]*(1-bbx0_mask[:, 0:1, :, :])) ** 2).sum()
    loss += lambda_noobj * (((predicts*obj_mask*(1-bbx0_mask))[:, 4, :, :]) ** 2).sum()
